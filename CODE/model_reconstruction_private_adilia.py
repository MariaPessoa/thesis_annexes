...
if __name__ == '__main__':
    ...
    if len(arg) > 0:
        ...
        config_dict = {
            ...
            'DATA_PATH': os.path.join(root_folder, 'gtexv8_4tissues_rin6_tpm_log10_globalqts_cnorm.csv'),
            "METADATA_PATH": os.path.join(root_folder, "metadata_clean.csv"),
            'CS_MODEL_DF_FOLDER': os.path.join(root_folder,"results"),
            'CS_MODEL_NAMES': 'b40_tinit_log10_csm_protect',
            'GENE_NAME_GRAB_PATTERN': 'ENSG[0-9]*',
            'PROTECTED_REACTION_LIST': 'biomass_human,HMR_10023,HMR_10024',
            'PROTECTED_REACTION_PATH': os.path.join(root_folder, "task_rcts_protected.csv"),
            "TASKS_PATH": os.path.join(root_folder,"Human-GEM","data","metabolicTasks", 'metabolicTasks_Essential.xlsx'),
            "TASK_RESULTS_PATH": os.path.join(root_folder,"results","b40_tinit_log10_tasks_protect.json"),
            'INDEX_COLUMNS': '0', #'0,1'
            'NTHREADS': '8',
    ...
    exp_data = pd.read_csv(config_dict['DATA_PATH'],header=0,index_col=0)
    ...
    for t in tissue_meta["Tissue"].unique():
        ...
        # sp = np.where(exp_data.index.str.contains("|".join(list(meta_indx)), case=False, regex=True))[0][:53]
    ...
    safe_threads = {'tinit': NTHREADS, 'fastcore': NTHREADS}
